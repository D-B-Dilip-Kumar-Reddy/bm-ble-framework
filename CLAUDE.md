...
|---|---|---|---|---|
| `POCKET_6K_G2` | Pocket Cinema Camera 6K G2 | v8.6 | In progress | **Primary reference.** The operator's physical unit was upgraded from v7.9 to v8.6 on 2026-07-27, so this is the only G2 firmware that can be tested against real hardware. **Phase 1 complete** (2026-07-28): GAP and device-info metadata are both `readable: true` here, unlike v7.9. **Phase 2 complete** (2026-07-29): `commands.recording` is `VERIFIED` — 3/3 echo-verified `CameraSession` cycles. Same coordinates as v7.9 but `reserved` is `0`, not `1` (both accepted; see `docs/recording.md`), a live example of why design principle 6 forbids inheriting. **Phase 3 in progress** (2026-07-30): step 9's passive sweep populated `codecs`, 7 of 8 `resolutions`, and the report-side coordinates of `codec_quality`/`recording_format` (all CANDIDATE). `video_format` + a seed `fps_modes` were then added as explicit CANDIDATE hypotheses to unblock step 10 (a probe packet can't be built without them). A follow-up dedicated fps sweep filled `fps_modes` to 7 of 8, with `60` initially recorded as a candidate ceiling after two silent sweeps — **retracted** after three standalone retries reported it cleanly twice, byte-identical, so `fps_modes` is now complete at 8/8 (`docs/settings.md` §18.7/§18.8, a live example of why a single silent window isn't enough to write a ceiling). Step 10's full `0x00`–`0x1F` `dimension_enum` sweep (2026-07-30) then confirmed all 8 enums, matching both `POCKET_6K_G2 v7.9`'s and `POCKET_6K_PRO v8.6`'s numbers exactly, and supplied HD's width/height that step 9 had missed; `commands.video_format` is now a coordinates-confirmed CANDIDATE (`docs/settings.md` §18.9). Outstanding: the ProRes/4K DCI enum gap (same open question as both other profiles, not yet investigated to their depth here), a confirming write for all three settings families (step 12), and the full `CameraSession` round trip (step 13). All Python defaults (`DEFAULT_FIRMWARE` in `tools/`, `FIRMWARE` in `examples/`) point here |
...
| `POCKET_6K_G2 v8.6` | **Phase 3 in progress** (2026-07-30) — the current primary reference, and the live worked example of the firmware-upgrade variant below. Step 9 done; step 10 done (all 8 `dimension_enums` confirmed, ProRes/4K DCI gap open like both other profiles); step 12 (confirming writes) next | `docs/recording.md` ("Per-camera status"), `docs/settings.md` §18, §18.7, §18.9 |
