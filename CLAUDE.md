...
| `docs/rest/transport.md` | **REST/WebSocket transport (8.6)** — why the endpoint sweep precedes any code, the USB-not-LAN transport story and host discovery over RNDIS, the gate questions the sweep must answer, and `tools/rest/probe_endpoints.py`'s two modes (read-only; opt-in idempotent same-value write probes, with destructive endpoints never probed). Sweep results are empty until run on real hardware |
