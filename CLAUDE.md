...
| `docs/ble/datetime.md` | BLE Category 7 (Real Time Clock/language/timezone) investigation — UNCONFIRMED; three passive real-hardware runs (`tools/sniffers/sniffer_datetime.py`'s `--actions` and `--burst-seconds` modes) all found zero Category 7 traffic, including a full connect-time state burst; `tools/control/send_datetime_command.py`'s active write probe is the current next step, not yet real-hardware-run; the planned tie-in to fixing `guess_new_still_path()`'s camera-clock-skew failure mode |
