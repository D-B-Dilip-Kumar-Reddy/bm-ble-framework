...
| `docs/ble/datetime.md` | BLE Category 7 (Real Time Clock/language/timezone) investigation — UNCONFIRMED, no real-hardware capture yet; motivation, capture procedure (`tools/sniffers/sniffer_datetime.py`), and the planned tie-in to fixing `guess_new_still_path()`'s camera-clock-skew failure mode |
