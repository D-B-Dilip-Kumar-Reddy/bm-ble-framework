...
| `docs/ble/datetime.md` | BLE Category 7 (Real Time Clock/language/timezone) investigation — UNCONFIRMED, 0-for-4 on real hardware; three passive runs (`tools/sniffers/sniffer_datetime.py`'s `--actions` and `--burst-seconds` modes) found zero Category 7 traffic including a full connect-time state burst, and a correctly-formed active write (`tools/control/send_datetime_command.py`) with default wire coordinates had no visible effect; `--reserved`/`--operation` override flags added as the next discovery axes, not yet tried; the planned tie-in to fixing `guess_new_still_path()`'s camera-clock-skew failure mode |
