...
| `docs/photo_capture.md` | Photo-capture reverse engineering — passive phase complete (2026-07-27, both cameras): a body-triggered still produces NO report at all. First active probe (G2, same day) came back **inconclusive**: every INT8 candidate tried was confirmed, which is itself a sign of an unreliable read, not a finding — see §6 for why and the redo protocol (independent on-camera counter verification, VOID tried first) before anything can enter a profile; also the connect-burst contamination hazard, the `0x09/0x02` remaining-time lead (not per-photo), and the open verification question |
