...
| `sensorResolution` writable? | **Open** — the format PUT itself works, but only a same-value body has been sent; a *changing* write to `sensorResolution` specifically is untried. `RestCameraSession.set_camera_format` (Phase 5, docs/rest/session.md) deliberately never touches this field, so it doesn't answer this row either — settling it needs a dedicated probe, not a side effect of the codec/resolution/fps write path |
