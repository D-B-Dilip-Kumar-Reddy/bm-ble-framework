...
| `PUT /media/devices/{deviceName}/doformat` implemented? | **Partially answered, `POCKET_6K_G2 v8.6`, 2026-08-13, across two runs.** Also `NEVER_WRITE`, so `tools/rest/probe_endpoints.py` itself still has no opinion. `examples/rest_format_device.py`'s first real-hardware run reached a real `PUT` and got `400 {"error": "Field 'filesystem' missing from request body."}` — disproving the spec's (`MediaControl.yaml`) own claim that `filesystem` is optional. A second run, with `filesystem` fixed, got `400 {"error": "Field 'volume' missing from request body."}` — the camera validates one field at a time, so the first run's "volume wasn't rejected, so it's optional" reading was wrong; it just hadn't been checked yet. `RestCameraSession.format_device()` now requires `filesystem` and resolves `volume` from the device's current name via `storage_state()` when not given. Neither run got past its own `400`, so whether a *correctly shaped* `PUT` actually formats the device (and the polling/completion path beyond it) remains unconfirmed — a rerun with both fixes is the next real-hardware step — see docs/rest/session.md's Phase 10 section |
