...
| `PUT /media/devices/{deviceName}/doformat` implemented? | **Partially answered, `POCKET_6K_G2 v8.6`, 2026-08-13.** Also `NEVER_WRITE`, so `tools/rest/probe_endpoints.py` itself still has no opinion. `examples/rest_format_device.py`'s first real-hardware run reached a real `PUT` and got a real response — `400 {"error": "Field 'filesystem' missing from request body."}` — disproving the spec's (`MediaControl.yaml`) own claim that `filesystem` is optional; `RestCameraSession.format_device()` now requires it. That run never got past the `400`, so whether a *correctly shaped* `PUT` actually formats the device (and the polling/completion path beyond it) remains unconfirmed — a rerun with the fix is the next real-hardware step — see docs/rest/session.md's Phase 10 section |
