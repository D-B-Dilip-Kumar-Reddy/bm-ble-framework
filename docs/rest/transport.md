...
| `PUT /media/devices/{deviceName}/doformat` implemented? | **Yes — confirmed, `POCKET_6K_G2 v8.6`, 2026-08-13, across three runs.** Also `NEVER_WRITE`, so `tools/rest/probe_endpoints.py` itself still has no opinion; this comes entirely from `RestCameraSession.format_device()`'s own real-hardware runs via `examples/rest_format_device.py`. Run 1: `400 {"error": "Field 'filesystem' missing from request body."}` — disproved the spec's (`MediaControl.yaml`) own claim that `filesystem` is optional. Run 2 (filesystem fixed): `400 {"error": "Field 'volume' missing from request body."}` — the camera validates one field at a time, so run 1's "volume wasn't rejected, so it's optional" reading was wrong. Run 3 (both fields resolved — `filesystem="ExFAT"`, note the real casing differs from the spec's `"ExFat"` example; `volume` defaulted from the device's current name): **`204`, followed by a full successful format** — `PUT` at `15:37:02.796`, `state` observed `"Mounted"` -> `"Formatting"` -> `"Mounted"`, complete at `15:37:07.871` (~5s for a 1TB card), `clip_count` `20` -> `0`, `remaining_space` restored. See docs/rest/session.md's Phase 10 section for the full write-up |
