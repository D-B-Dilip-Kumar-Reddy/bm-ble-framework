...
| `PUT /media/devices/{deviceName}/doformat` implemented? | **Unknown** — also `NEVER_WRITE`, and unlike `record`, **not yet exercised by `RestCameraSession.format_device()` (Phase 10) either.** Only the GET side (`.../doformat` returning a format key, `.../doformatSupportedFilesystems`) is sweep-confirmed. Real PUT behavior rests entirely on the official spec (`MediaControl.yaml`) being accurate — `examples/rest_format_device.py`'s first real-hardware run is what would confirm it, and hasn't happened yet — see docs/rest/session.md's Phase 10 section |
