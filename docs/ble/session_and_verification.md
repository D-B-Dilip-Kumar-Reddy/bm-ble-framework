...
## Verification strategy (moved from CLAUDE.md)
...
## `CameraSession` (`src/bmd_camera/ble/session.py`)
...
on an unpopulated profile. See `docs/ble/payload_profiles.md` for the profile
structure.
...
`docs/ble/settings.md` (which also tabulates exactly which decoded payload
...
(`docs/ble/settings.md` §8), `set_codec_quality` and `set_recording_format`
via one genuine cycle each, surfaced by `set_camera_format`'s proxy path
(`docs/ble/settings.md` §10). Two deviations from the recording flow:
...
- **`set_video_format` arms three echo channels.** Its own `0x01/0x00`
  ...
  **Why three, not two — a real bug found on real hardware
  (`docs/ble/settings.md` §10):** the mode-notify payload encodes
  ...
  confirmed since `docs/ble/settings.md` §8) carried real confirmation the
...
`docs/ble/settings.md` §8, §10, §11.

**The same no-echo-on-redundant-write behavior was confirmed for
`video_format` and `recording_format` too (2026-07-21, `docs/ble/settings.md`
...
the state. See `docs/ble/settings.md` §14.
...
evidence behind the two-step workaround: `docs/ble/settings.md` §9.
...
`docs/ble/payload_profiles.md`): a codec listed there for that resolution is one
...
(`docs/ble/settings.md` §16) is the first and so far only entry. If the
...
`resolution_spec.max_fps_int` (added 2026-07-24, see `docs/ble/payload_profiles.md`)
...
(`timecode.decode_timecode` — see `docs/ble/timecode.md`) via a private callback.
...
`docs/ble/timecode.md` for why), or `None` if `last_stop_timecode` is missing.
...
before BCD-decoding the payload. See `docs/ble/timecode.md` for the confirmed
wire format.

### Camera-initiated stop detection

See `docs/ble/recording.md`'s "Camera-initiated stop detection" section for the
...
`docs/ble/recording.md`'s "A CANDIDATE signal: the write-margin warning") that's
...
see `docs/ble/recording.md` — so attempting one would otherwise raise a
...
(`docs/ble/photo_capture.md` §7, §9) has neither channel on **either**
...
(`docs/ble/photo_capture.md` §7.3): verify out-of-band over USB instead of
...
directly — see `docs/ble/timecode.md`.
