...
| `POCKET_6K_PRO v8.6` | `CANDIDATE` | See `docs/ble/command_discovery.md` |
...
both cameras (`docs/ble/photo_capture.md` §7/§9).
...
1. It is the only reserved value with a clean wire echo for **both** outcomes
   ...
   race in `docs/ble/command_discovery.md`'s safety model.
...
matching `docs/ble/timecode.md`'s finding that the camera resets it on record
...
state instead of surfacing a traceback (`docs/ble/command_discovery.md`); the
...
transport-mode semantics, see `docs/ble/protocol.md` §6). `value: int` is therefore an explicit, required
parameter sourced from the profile's `commands.recording.values` map
(`{"start": 2, "stop": 0}` — see `docs/ble/payload_profiles.md`), never assumed
...
1. Subscribe to `INCOMING_CONTROL` and start buffering **before** sending the
   command (`NotificationRouter.arm()` — see `docs/ble/session_and_verification.md`).
...
`docs/ble/packet_structure_and_constants.md`) found exactly one
...
(see `docs/ble/active_camera_control.md`) has since closed this gap: a real run
...
### Storage Media Monitoring — full design intent *(planned)*

This subsection is design intent, moved verbatim from CLAUDE.md — no storage
monitoring is implemented yet (`StorageState`/`CameraState` do not exist; see
design principle 4). It is broader than the record-start/stop precondition
above: the full subsystem also covers photo capture and playback.

**Scope: the SD card slot.** The CFast slot may be added later; **external
USB media is out of scope entirely**. Code must not assume a single media
device anyway — the camera reports a working set with an active member, so
resolve the active device rather than indexing slot 0, and adding CFast
becomes a data change rather than a code change.

Storage state is read on connect and updated from `CAMERA_STATUS`
notifications. It is tracked in `StorageState` (part of `CameraState`) and
covers:
...
How storage state gates each operation:
...
If the pre-condition fails, raise `BMDStorageError` immediately — do not
attempt the command.
...
capture (`docs/ble/settings.md` §6) contained one `category=0x09 parameter=0x01`
...
**Follow-up, now in clean captures (2026-07-20, `docs/ble/settings.md` §7):**
...
2. ~~Add the confirmed values to `payloads/models/POCKET_6K_G2_v7.9.json`.~~
   ...
   `echo_operation=2` — see `docs/ble/payload_profiles.md` for the structure).
...
5. ~~Build a send-then-observe mode, and confirm the echo on real
   ...
   `docs/ble/active_camera_control.md`) sent `record_start`/`record_stop`
...
6. ~~Wire `encode_record_start` / `encode_record_stop` /
   ...
   Done — see `docs/ble/session_and_verification.md` and
...
8. ~~Capture the clip length.~~ Done — `CameraSession` tracks the `TIMECODE`
   ...
   `last_clip_duration_seconds()` (see `docs/ble/timecode.md`). The real wire
   ...
   latest reading — see `docs/ble/timecode.md` for the bug this fixed. Duration
