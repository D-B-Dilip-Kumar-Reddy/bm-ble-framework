...
see `docs/ble/active_camera_control.md`). It exists to serve CLAUDE.md's
...
(`bmd_camera.ble.camera_controller.<model_key>`) to `DEBUG` in `__init__`, so a
...
`POCKET_6K_PRO v8.6`, `docs/ble/photo_capture.md` §5.2): after connect, both
...
`docs/ble/settings.md`'s CANDIDATE families describe (codec to ProRes/back,
quality variant, resolution, FPS). Known passive limit (confirmed on the
2026-07-20 G2 run, `docs/ble/settings.md` §5): video_format dimension enums
...
contrast and keep everything. See `docs/ble/photo_capture.md`.
...
reported (docs/ble/photo_capture.md §8). First runs 2026-07-27, both cameras:
...
see `docs/ble/photo_capture.md` §10.1/§10.3 for the full results, including a
