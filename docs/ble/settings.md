...
write known to fail (`docs/ble/payload_profiles.md`, `docs/ble/session_and_verification.md`).
This still
...
`resolutions`, and `fps_modes` (see `docs/ble/payload_profiles.md` for the
...
(`docs/ble/session_and_verification.md`'s `connect_settle_s`, chosen because
...
already has (see `docs/ble/session_and_verification.md`); it's now fixed with
...
`docs/ble/command_discovery.md`'s safety model, not yet fixed there since every
...
CANDIDATE signal `docs/ble/recording.md` correlates with a camera-initiated
...
`docs/ble/recording.md`'s write-margin section for the full reasoning. Worth
...
`docs/ble/photo_capture.md` §10.1's first Sensor Area capture (2026-07-27,
...
this time cross-model: `docs/ble/photo_capture.md` §10.3's PRO rerun of the
...
causation: `docs/ble/photo_capture.md` §10.4's PRO-only interleaved A-B-A-B
...
115-page official protocol PDF (`docs/ble/photo_capture.md` §10.6, every
...
**Read-only, confirmed 2026-07-27** (`docs/ble/photo_capture.md` §10.7, the
...
from a completely different evidence channel: `docs/ble/photo_capture.md` §8.1
...
`docs/ble/recording.md`) read `low_margin`/`-2` in essentially every one of
...
`docs/ble/recording.md` documents for `record_stop()` (a redundant stop while
...
No change to the signal's modeling; see `docs/ble/recording.md`.
...
`docs/ble/session_and_verification.md`) discards the genuinely fresh report.
...
`docs/ble/session_and_verification.md` for why the router's duplicate filter
...
`is_recording is False` early return (`docs/ble/recording.md`) exactly.
...
| Session methods | `session.py` `set_codec_quality` / `set_video_format` / `set_recording_format` (see `docs/ble/session_and_verification.md` for the echo strategy), plus the `set_camera_format` orchestration (§9) and its `_closest_reachable_resolution` helper |
...
(2026-07-21, see its module docstring and `docs/ble/active_camera_control.md`)
...
**Hardening applied**, all in `src/bmd_camera/ble/session.py`:
...
**Read `docs/ble/protocol.md` §4 before running this.** The official spec's stated
...
behavior before (`docs/ble/protocol.md` §3) — but don't read a failure from that test
...
storage telemetry (`0x09/0x01`'s "write margin" signal, `docs/ble/recording.md`) has
...
`docs/ble/payload_profiles.md` for the schema field's design), and
...
`docs/ble/session_and_verification.md`). This is deliberately framed as a *software*
...
proof of the same underlying cause: `docs/ble/photo_capture.md` §10.2 records
...
see `tools/control/sweep_camera_format.py` (`docs/ble/active_camera_control.md`) —
...
Unlike the G2 v8.6 profile's equivalent gap (`docs/ble/settings.md` §18.14), this section's
...
`resolutions.<name>.max_fps_int`, was added for it (`docs/ble/payload_profiles.md`
...
`docs/ble/session_and_verification.md` and encountered repeatedly throughout
...
`docs/ble/photo_capture.md` §10.1. It is also why a `frame_flags` value
...
3. **No `HD` resolution.** Its window (`res_prores_hd`) was the first of its
   ...
   `docs/ble/command_discovery.md`'s safety model. `1920x1080` was **not**
...
(`docs/ble/photo_capture.md` §10.5), here appearing in a passive capture instead.
...
1. **`commands.video_format`** — category `1` / parameter `0`, the officially
   documented FORMAT packet coordinates (`docs/ble/protocol.md`), made plausible
...
first exhausting retries — exactly the lesson `docs/ble/payload_profiles.md`'s
...
positive, `docs/ble/photo_capture.md` §10.5):
...
against real PRO hardware (`docs/ble/session_and_verification.md`) — a `0x0C`
...
- The bit is not a static property of the resolution at all, but reflects a
  ...
  already-closed Sensor Area investigation (`docs/ble/photo_capture.md`
...
- One or more of the four readings reflects stale state from before the
  ...
  (`docs/ble/photo_capture.md` §10.5) — though nothing here has been confirmed
...
#    target payload — see docs/ble/protocol.md §4's OFFSET semantics) via
...
`connect()` retry loop built earlier this session (`docs/ble/winrt_ble_connection_hardening.md`)
...
directly (`docs/ble/photo_capture.md` §8) — so for BRAW, the sensor's actual
...
closed on other camera/firmware combinations (`docs/ble/photo_capture.md`
...
`docs/ble/photo_capture.md` §10, which closed the BLE search as having no write path by any
means tried.
