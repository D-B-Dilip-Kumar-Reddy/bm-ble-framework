...
A. Seed      (category, parameter, data_type) — from a saved passive capture
             (--from-capture) or CLI flags; docs/ble/protocol.md's [spec] tables
...
D. Emit      save the capture as evidence; print a ready-to-paste
             payloads/ble_schema.json-shaped "commands" block
```
...
five int8 elements, recording_format's five int16s — `docs/ble/settings.md`).
...
produce no report at all to seed from — `docs/ble/photo_capture.md` §5, the
...
`docs/ble/photo_capture.md` §3). Build it only when a sweep actually needs it.
...
| `tools/control/discover_command.py` | Interactive driver — prompts, BLE session, sweep loop | Manual (matches `docs/ble/sniffer_capture_engine.md`'s stance on interactive tools) |
...
- `seed_triples_from_capture(capture_dict, exclude_ambient=True)` — parses
  ...
  **Known limitation:** the rescue is payload-shaped, so it does not help a
  ...
  (`docs/ble/recording.md`'s "Camera-initiated stop detection"), not through
...
- `build_command_block(name, confirmed, capture_ref, discovered_on)` —
  ...
  tests validate its output against the real `payloads/ble_schema.json`, so
...
G2's first photo-capture sweep (2026-07-27, `docs/ble/photo_capture.md` §6)
...
`docs/ble/photo_capture.md` §6.3 for the concrete redo protocol this produced.
...
(`docs/ble/photo_capture.md` §7). The crash still meant no block could be
...
(`docs/ble/photo_capture.md` §9), hit the exact same warning/crash/manual-
...
(`docs/ble/recording.md`). When resolving one of these by hand, prefer the
...
for — see `docs/ble/session_and_verification.md`). `tools/control/
send_settings_command.py` hit exactly this on real hardware 2026-07-20
(`docs/ble/settings.md` §6): its first three captures showed the initial burst
...
discovery sweep** (`docs/ble/photo_capture.md` §6.1): candidate 1's first send
landed mid-burst (the `0x0C` lens-string cadence from
`docs/ble/sniffer_capture_engine.md`'s connect-burst section), and the operator
...
`docs/ble/settings.md` §13 and `docs/ble/active_camera_control.md`). This tool has
...
(2026-07-23, `docs/ble/settings.md` §3/§16, later ruled out on real hardware),
a `--video-format-extra E1 E2` override (2026-07-24, `docs/ble/settings.md`
§16, also ruled out) for probing `video_format`'s unexplained trailing
payload elements, an `--operation NAME` override (2026-07-24,
`docs/ble/settings.md` §16, tried on real hardware for the absolute-payload
...
VALUE [VALUE ...]` override (2026-07-24, `docs/ble/settings.md` §16, tried on
...
to test `OFFSET`'s documented delta semantics (`docs/ble/protocol.md` §4),
...
gap is now exhausted (`docs/ble/settings.md` §16) and accepted as a guarded
software capability gap (`resolutions."4K DCI".known_unreachable.ProRes`,
`docs/ble/payload_profiles.md`). `discover_command.py`
...
(2026-07-24, `docs/ble/active_camera_control.md`), exists precisely because that
...
Its first real-hardware run (2026-07-24, `docs/ble/settings.md` §17) found a
...
`docs/ble/protocol.md`'s `[spec]` tables.)
...
This validates the pasted block against `payloads/ble_schema.json` immediately
...
the latter omitting `values`) against the real `payloads/ble_schema.json` —
