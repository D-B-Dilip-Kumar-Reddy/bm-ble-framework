...
plus per-candidate operator confirmation — see `docs/ble/command_discovery.md`)
and `tools/control/send_settings_command.py` (sends CANDIDATE-provenance
settings commands — see below and `docs/ble/settings.md`).

---

## `BMDCameraController.write_outgoing_control` (`src/bmd_camera/ble/camera_controller.py`)
...
`run_capture_windows` uses — see `docs/ble/sniffer_capture_engine.md`), marks the
...
`payloads/models/POCKET_6K_G2_v7.9.json` and `docs/ble/recording.md`), connects,
sends `record_start`, waits `--hold-seconds` (so something is actually
recorded), sends `record_stop`, and saves the combined capture.

This is the tool that closes `docs/ble/recording.md`'s previously-deferred
...
`docs/ble/settings.md`) built entirely from the profile's command blocks plus
...
`POCKET_6K_G2 v8.6` (`docs/ble/settings.md` §18.10) have since confirmed all
...
`docs/ble/settings.md`'s verification runbook, including the two-run experiment
...
resulting width/height (`docs/ble/settings.md` §4.1, §5).
...
prevent (`docs/ble/session_and_verification.md`), which this standalone tool
...
confirmation, independent of the bad capture) is in `docs/ble/settings.md` §6.
`tools/control/discover_command.py` has the same latent risk on its first
candidate only — see `docs/ble/command_discovery.md`'s safety model.
...
is already at produces no echo at all (`docs/ble/settings.md` §11 —
...
share that same silent-no-op behavior is an open question (`docs/ble/settings.md`
...
Added 2026-07-23 (`docs/ble/settings.md` §3/§4/§16). The `recording_format`
...
(`docs/ble/settings.md` §16). The write-byte axis is exhausted.

**`--video-format-extra E1 E2` — probe `video_format`'s unexplained trailing
elements.** Added 2026-07-24 (`docs/ble/settings.md` §16). `video_format`'s payload is
...
either; see `docs/ble/settings.md` §16 for the full write-up and what's left to try.

All three original candidate hypotheses for the PRO's ProRes/4K DCI gap are now
exhausted. A full-channel decode of the passive-capture evidence (`docs/ble/settings.md`
...
(`docs/ble/settings.md` §16). Every write attempted across all three exhausted
...
settings.md` §16). That doesn't itself rule `OFFSET` out — `docs/ble/protocol.md` §4
...
Added 2026-07-30 (`docs/ble/settings.md` §18.6). Packet header byte 3 was the one axis
...
axis (`docs/ble/settings.md` §3).

That is not hypothetical. On `POCKET_6K_G2 v8.6` the recording family accepts **both**
`0x00` and `0x01` (`docs/ble/recording.md`), every report on that firmware carries `0x00`,
...
lookup table.** Added 2026-07-24 (`docs/ble/settings.md` §16, `docs/ble/protocol.md` §4).
...
confirming echo. See `docs/ble/settings.md` §16 for the full write-up and the next
...
category/parameter tried so far. See `docs/ble/settings.md` §16 for the full write-up.
...
demonstrably takes effect (`docs/ble/settings.md` §15), so an operator
...
`docs/ble/settings.md` §7-§8), minus every `dimension_enum` value already
...
Motivating case: `docs/ble/settings.md` §16 — `POCKET_6K_PRO v8.6` has no known
...
A second, structurally different use case (`docs/ble/photo_capture.md` §10.1's
...
`POCKET_6K_PRO v8.6`, 2026-07-27 (`docs/ble/photo_capture.md` §10.5): the
...
mechanism `docs/ble/settings.md` §7 already established for `video_format`
...
capability gap (`resolutions.<name>.known_unreachable`, `docs/ble/payload_profiles.md`)
...
investigation eventually required before being accepted (`docs/ble/settings.md`
...
`POCKET_6K_PRO v8.6`, `docs/ble/settings.md` §17):** 448 combinations, one
...
Full write-up: `docs/ble/settings.md` §17.
...
no report to seed from (`docs/ble/photo_capture.md`). Full writeup:
